...
const meta: Meta<typeof Tooltip> = {
	...
	parameters: {
		...
		docs: {
			description: {
				component: `
Tooltip provides contextual information when users hover over or focus on an element. It is built on the platform — the Popover API for top-layer placement and CSS anchor positioning for placement — and is designed for short, helpful text content.
...
export const Sides: Story = {
	render: () => (
		<div style={{ display: 'flex', gap: '3rem', padding: '5rem' }}>
			{(['top', 'right', 'bottom', 'left'] as const).map((side) => (
				<Tooltip key={side} side={side} delayDuration={0}>
					<TooltipTrigger>{side}</TooltipTrigger>
					<TooltipContent>Placed on the {side}</TooltipContent>
				</Tooltip>
			))}
		</div>
	),
	parameters: {
		docs: {
			description: {
				story:
					'Placement is CSS anchor positioning where the engine supports it, and a small JS fallback where it does not — below Safari 26, Firefox 147 or Chrome 125. Both paths place the tooltip on the requested side; only the CSS path flips it away from a viewport edge.',
			},
		},
	},
}
...
export const CustomTiming: Story = {
	...
	parameters: {
		docs: {
			description: {
				story:
					'Control the timing of tooltip appearance with the `delayDuration` prop. Default value is 700ms.',
			},
		},
	},
}
...
export const Positioning: Story = {
	...
	parameters: {
		docs: {
			description: {
				story:
					'TooltipContent supports different positioning with `side` (top, right, bottom, left) and `align` (start, center, end) props. Tooltips automatically adjust position to stay within viewport.',
			},
		},
	},
}
...
export const AsChildTrigger: Story = {
	...
	parameters: {
		docs: {
			description: {
				story:
					'Use the `asChild` prop to render any custom element as the trigger. This is useful for maintaining semantic HTML while adding tooltip functionality.',
			},
		},
	},
}
		...
		<div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
			<div>
				...
				<p style={{ marginBottom: '16px', fontSize: '14px', color: '#666' }}>
					Use Tab to focus triggers, then wait for tooltip to appear. Press
					Escape to close any open tooltip.
				</p>
...
export const CommonUseCases: Story = {
			...
			<div>
				...
				<div style={{ width: '200px' }}>
					<Tooltip>
						...
						<TooltipContent>
							<p>
								This is a very long text that gets truncated when the container
								is too small
							</p>
						</TooltipContent>
					</Tooltip>
	...
	parameters: {
		docs: {
			description: {
				story:
					'Common use cases for tooltips include form field help, icon button labels, and showing full text for truncated content.',
			},
		},
	},
}
