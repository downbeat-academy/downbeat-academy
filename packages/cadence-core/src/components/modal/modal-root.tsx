...
/**
 * State container shared by `Dialog` and `Drawer`.
 *
 * Renders no element of its own — the `<dialog>` lives in the top layer, so there is
 * nothing for a wrapper to position or contain.
 */
const ModalRoot = ({
	children,
	open,
	...
	// mode, so a controlled modal whose owner ignores `onOpenChange` stays exactly where
