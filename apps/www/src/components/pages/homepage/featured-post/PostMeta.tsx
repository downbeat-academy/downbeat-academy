...
const PostMeta = ({ authors, date }) => {
  ...
  const getAuthorImages = authors.slice(0, 3).map(author => {
    const image = <Image src={getSanityImageUrl(author.avatar.image.asset)} alt={author.name} width={100} height={100} />
  ...
  return (
    <div className={s.wrapper}>
      <AvatarGroup avatars={getAuthorImages} isInteractive={authors > 1 ? true : false} />
